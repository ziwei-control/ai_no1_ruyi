...
class RuyiScheduler:
    ...
    def run_plugin_chain(self, parsed: Dict) -> Dict:
        ...
        # Step 1: 注册标准插件
        self.register_plugin('help', self._help_plugin)
        ...
        self.register_plugin('install', self._install_plugin)
        self.register_plugin('help', self._help_plugin)
        ...
        # 依次运行插件
        plugins_order = ['help', 'nlp', 'path', 'install', 'exec', 'report', 'evolve']
        for plugin_name in plugins_order:
            context = self.run_plugin(plugin_name, context)
            if 'error' in context:
                break
            if context.get('help_shown'):
                # 帮助已显示，退出
    ...
    def _help_plugin(self, context: Dict) -> Dict:
        """Help 插件: 如意帮助用户了解自己"""
        parsed = context['parsed']
        if parsed['command'] != 'help':
            return context
        ...
        print(f"")
        print(f"✨ 如意核心特征：")
        print(f"  🤖 善良的灵魂 - 主动帮助用户")
        print(f"  🤝 乐于助人 - 自动补全参数，智能识别")
        print(f"  🛠️  自助安装能力 - 缺少工具可以自行安装")
        print(f"  🔒  最高权限支持 - 可以调用 sudo 等高权限指令")
        print(f"")
        print(f"🚀 如意支持的命令类型：")
        print(f"  📜 脚本测试 - 测试 /path/to/script.sh")
        print(f"  🔤 命令测试 - 测试 echo/ls/cat")
        print(f"  🐳 Docker 测试 - 测试 docker run nginx:latest")
        print(f"  📦 自助安装 - 如意会自动检测包管理器并安装缺失工具")
        print(f"")
        print(f"⚙️  参数说明：")
        print(f"  --次数=N         测试次数，默认3次")
        print(f"  --超时=N         单次超时时间，默认120秒")
        print(f"  --熔断=N         连续失败N次后熔断")
        print(f"  --verbose        详细模式，显示每轮输出")
        print(f"")
        print(f"💡 使用示例：")
        print(f"  '测试这个项目，测3次直到成功'")
        print(f"  '/root/.copaw/scripts/test.sh --次数=3 --超时=120'")
        print(f"  '测试 echo hello，测3次直到成功'")
        print(f"")
        print(f"✨ 如意愿意为你效劳！")
        print(f"")
    ...
    def _install_plugin(self, context: Dict) -> Dict:
        """Install 插件: 自助安装能力 - 缺少工具可以自行安装"""
        if not self.controller.auto_install:
            print(f"📦 Install 插件: 自助安装已禁用")
            return context
        ...
        if cmd and not shutil.which(cmd):
            ...
            # 🌟 如意自助安装
            if self.controller.kind_hearted:
                print(f"   💡 如意建议：自动尝试安装 {cmd}")
                
                # 尝试安装
                if self._install_tool(cmd, verbose=verbose):
                    print(f"   ✅ 自助安装成功: {cmd}")
                    # 重新检查
                    if shutil.which(cmd):
                        task.metadata['command_type'] = 'system'
                        task.metadata['installed'] = True
                    else:
                        context['error'] = f"安装后仍无法找到: {cmd}"
                        return context
                else:
                    print(f"   ❌ 自助安装失败: {cmd}")
                    context['error'] = f"自助安装失败: {cmd}"
                    return context
            else:
                context['error'] = f"命令不存在且自助安装已禁用: {cmd}"
                return context
        elif cmd:
            print(f"   ✅ 命令已存在: {cmd}")
    ...
    def _install_tool(self, tool_name: str, verbose: bool = False) -> bool:
        """安装工具 - 智能检测包管理器"""
        print(f"   🛠️  开始安装: {tool_name}")
        
        # 首先检查是否有 sudo 权限
        sudo = "sudo " if self.controller.sudo_enabled else ""
        
        # 检测包管理器
        package_managers = [
            ("apt-get", "install", tool_name),
            ("apt", "install", tool_name),
            ("yum", "install", tool_name),
            ("dnf", "install", tool_name),
            ("pacman", "-S", tool_name),
            ("brew", "install", tool_name),
            ("pip3", "install", tool_name),
            ("pip", "install", tool_name),
            ("npm", "install", f"-g {tool_name}" if not tool_name.startswith("@") else tool_name),
        ]
        
        import shutil
        
        for pm, action, args in package_managers:
            if shutil.which(pm):
                print(f"   📦 检测到包管理器: {pm}")
                
                # 构建命令
                if pm in ["pip3", "pip"]:
                    cmd = f"{pm} install {args}"
                elif pm == "npm":
                    cmd = f"{pm} {action} {args}"
                else:
                    cmd = f"{sudo}{pm} {action} {args} -y"
                
                if verbose:
                    print(f"   📝 执行命令: {cmd}")
                
                result = self.execute_command(cmd, timeout=120, verbose=verbose)
                
                if result.status == ExecutionStatus.SUCCESS:
                    print(f"   ✅ 安装成功: {tool_name}")
                    return True
                else:
                    print(f"   ⚠️  {pm} 安装失败，尝试其他包管理器...")
        
        # 如果包管理器安装都失败了，尝试源码安装
        if tool_name in ["git"]:
            # git 可以源码安装
            print(f"   🌐 尝试源码安装 {tool_name}...")
            git_install_cmd = """
                apt-get update -qq && apt-get install -qq -y git
            """
            result = self.execute_command(git_install_cmd, timeout=180, verbose=verbose)
            if result.status == ExecutionStatus.SUCCESS:
                print(f"   ✅ 源码安装成功: {tool_name}")
                return True
        
        print(f"   ❌ 无法安装: {tool_name}")
        return False
